...
const splitesCode = ref(Array.from({ length: props.digits }, () => ''));
...
watchEffect(() => {
    if (props.error !== null) {
        splitesCode.value=Array.from({ length: props.digits }, () => '')
        focusInput(0)
    }
})
...
const handleInput = (index, event) => {
    ...
    if (!isNaN(value) && value !== '') {
        ...
        splitesCode.value[index - 1] = '';
        if (index > 0) {
            focusInput(index - 1);
        }
    }
    emit('update:modelValue', splitesCode.value.join(''));
};
</script>
...
<template>
    <div class="flex mb-2 space-x-2 rtl:space-x-reverse">
        <TextInput
            v-for="(code, index) in splitesCode"
            :key="index"
            :id="`code-${index + 1}`"
            :value="code"
            v-model="splitesCode[index]"
            :type="'text'"
            :name="`code-${index + 1}`"
